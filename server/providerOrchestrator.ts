...
/**
 * Provider fallback orchestrator — ported from src/convex/providerOrchestrator.ts.
...
import { rankCandidates } from "./services/optimus";
import { recordPerformance, reportProviderResult, reserveProviderCapacity } from "./services/rateLimits";
...
export async function attemptWithProviderFallback<T>(
  ...
  for (let attempt = 0; attempt < attemptCandidates.length && !success; attempt++) {
    ...
    try {
      const capacity = await reserveProviderCapacity(
        candidate.provider,
        candidate.modelId,
        Math.ceil((systemPrompt.length + userContent.length) / 4) + maxTokens,
      );
      ...
      const projectedNeurons = Math.ceil((systemPrompt.length + userContent.length) / 4) + maxTokens;
      await reportProviderResult({
        provider: candidate.provider,
        model: candidate.modelId,
        status: 200,
      ...
      if (err instanceof ProviderRequestError) {
        ...
        const reportArgs: Parameters<typeof reportProviderResult>[0] = {
          provider: candidate.provider,
          model: candidate.modelId,
          status: err.status,
        ...
        if (retryAfter !== undefined && retryAfter > 0) {
          reportArgs.cooldownSeconds = retryAfter;
        } else if (err.status === 429 || err.status === 503) {
          reportArgs.cooldownSeconds = computeBackoffCooldown(attempt, deadlineAt);
        }

        await reportProviderResult(reportArgs);
      }

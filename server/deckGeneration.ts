...
/**
 * Deck generation pipeline — ported from src/convex/deckGeneration.ts.
 * The two Convex actions become plain async functions invoked by REST routes.
 */
...
import { getAdaptiveSettings } from "./services/rateLimits";
...
export type DeckGenerationArgs = {
  ...
  deviceToken?: string;
  /** Client IP, resolved from the request by the route handler. */
  ip?: string;
};
...
export async function generateDeckFromDocument(args: DeckGenerationArgs): Promise<GenerateDeckResult> {
  ...
  const { candidates, providerCount } = await getCandidateChain(args.preferredProvider);
  ...
  await enforceIpRateLimit(ip, estimatedTokens, creatorDeviceIdHash);
  ...
  try {
    for (let i = 0; i < chunks.length; i++) {
      ...
      const res = await attemptWithProviderFallback({
        ...
        onSuccess: async (parsed, candidate, content, usage) => {
          await recordUsage(args.jobId, "document", candidate, systemPrompt, userContent, content, usage, ip, creatorDeviceIdHash);
        ...
        updateJob: (patch) => updateJob(args.jobId, patch, keySeed),
        assertJobActive: () => assertJobActive(args.jobId),
        context: { ...(args.jobId !== undefined && { jobId: args.jobId }), deadlineAt, estimatedSeconds, sectionIndex: i, totalSections, kind: "document" as const }
      });

      if (!res.success && res.lastErr) {
        warnings.push(`section ${i + 1}: ${res.lastErr instanceof Error ? res.lastErr.message : String(res.lastErr)}`);
      }
      await appendFallbackTrail(args.jobId, res.fallbackTrail, keySeed);
    ...
    for (let pass = 0; pass < completionPasses && allCards.length < requestedCount; pass++) {
      ...
      const res = await attemptWithProviderFallback({
        ...
        parser: parseAiDeckGeneration,
        onSuccess: async (parsed, candidate, content, usage) => {
          await recordUsage(args.jobId, "document", candidate, completionSystemPrompt, completionUserContent, content, usage, ip, creatorDeviceIdHash);
          parsedCardCount += parsed.cards.length;
        ...
        updateJob: (patch) => updateJob(args.jobId, patch, keySeed),
        assertJobActive: () => assertJobActive(args.jobId),
        context: { ...(args.jobId !== undefined && { jobId: args.jobId }), deadlineAt, estimatedSeconds, sectionIndex: chunks.length, totalSections, kind: "document" as const }
      });
  ...
  if (allCards.length === 0) {
    ...
    await recordGenerationTelemetry("generation_failed", {
      jobId: args.jobId,
      kind: "document",
      requestedCount,
      generatedCount: 0,
  ...
  await recordGenerationTelemetry("generation_completed", {
    jobId: args.jobId,
    kind: "document",
    requestedCount,
...
export type PromptGenerationArgs = {
  prompt: string;
  deckName?: string;
  cardCount?: number;
  difficulty?: "beginner" | "intermediate" | "advanced";
...
export async function generateDeckFromPrompt(args: PromptGenerationArgs): Promise<GenerateDeckResult> {
  ...
  const res = await attemptWithProviderFallback({
    ...
    parser: parseAiDeckGeneration,
    onSuccess: async (parsed, candidate, content, usage) => {
      await recordUsage(args.jobId, "prompt", candidate, systemPrompt, userContent, content, usage, ip, creatorDeviceIdHash);
    ...
    updateJob: (patch) => updateJob(args.jobId, patch, keySeed),
    assertJobActive: () => assertJobActive(args.jobId),
    context: { ...(args.jobId !== undefined && { jobId: args.jobId }), deadlineAt, estimatedSeconds: etaSeconds, kind: "prompt" as const }
  });
  ...
  if (!res.success) {
    if (isGenerationCanceledError(res.lastErr)) {
      await recordGenerationTelemetry("generation_canceled", {
        jobId: args.jobId,
        kind: "prompt",
        requestedCount,
        metric: 1,

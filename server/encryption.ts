 ...
 * history). Set ENCRYPTION_PEPPER in the Function environment (neon.ts `env`
 * + `neon deploy --env <file>`) to a long random string.
 */
function getPepper(): string {
  const pepper = process.env.ENCRYPTION_PEPPER;
  if (!pepper) {
    throw new Error(
      "ENCRYPTION_PEPPER is not configured. Set it in the Function environment to a long random string.",
    );
  }
  return pepper;
}

...
function openRouterExtraHeaders(): Record<string, string> {
  return {
    "HTTP-Referer":
      process.env.OPENROUTER_APP_URL || process.env.PUBLIC_APP_URL || "http://localhost:5173",
  ...
  try {
    ...
    const models = (response.data ?? [])
      ...
      .map((model) => ({
        ...
        headers: createHeaders(apiKey, openRouterExtraHeaders()),
      }));
    ...
    const fallback: AiModelCandidate[] = [
      {
        ...
        supportsJsonMode: true,
        baseUrl: "https://openrouter.ai/api/v1",
        headers: createHeaders(process.env.OPENROUTER_API_KEY || "", openRouterExtraHeaders()),
      },
    ];

  ...
  return (
    <section className="nb-border bg-card nb-shadow p-5">
      <div className="flex items-center gap-3 mb-4">
        <div className="nb-border bg-muted p-2">
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4 mb-4">
        {[
          { label: "Cards Requested", value: telemetry.requested.toLocaleString(), icon: FileText, tint: "bg-muted", accent: "text-status-info" },
          { label: "Cards Generated", value: telemetry.generated.toLocaleString(), icon: CheckCircle, tint: "bg-muted", accent: "text-status-ok dark:text-status-ok" },
          { label: "Duplicates", value: telemetry.duplicates.toLocaleString(), icon: XCircle, tint: "bg-status-warn/10", accent: "text-muted-foreground dark:text-status-warn" },
          { label: "Total Duration", value: `${Math.round(telemetry.durationMs / 1000)}s`, icon: Clock3, tint: "bg-muted", accent: "text-muted-foreground" },
        ].map((item) => {
        ...
        <div className="nb-border-2 bg-muted/20 p-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <p className={`font-bold ${telemetry.parseFailures > 0 ? 'text-status-warn' : 'text-status-ok'}`}>{telemetry.parseFailures}</p>
            </div>
            <div>
